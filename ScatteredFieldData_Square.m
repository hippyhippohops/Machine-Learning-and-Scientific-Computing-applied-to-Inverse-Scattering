...
% Define the range of x and y values and our Grid G
x = linspace(-2, 2, 150);
y = linspace(-2, 2, 150);
size(x);
size(y);
...
ball = zeros(size(X)); 
ball(abs(X - center_of_scatterer(1,1) + Y - center_of_scatterer(2,1))+ abs(X - center_of_scatterer(1,1) - Y + center_of_scatterer(2,1)) <= radius_of_scatterer) = permitivitty_of_scatterer;
...
contrast(abs(X - center_of_scatterer(1,1) + Y - center_of_scatterer(2,1))+ abs(X - center_of_scatterer(1,1) - Y + center_of_scatterer(2,1)) <= radius_of_scatterer) = permitivitty_of_scatterer; % define a square here
...
%% Plotting Real Scattered Field Data
...
%% Plotting Complex Scattered Field Data
